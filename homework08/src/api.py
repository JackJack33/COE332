...
def get_gene_data(hgnc_id: str) -> Dict[str, str]:
    ...
    try:
        ...
        logging.error(json.loads(data.decode())['gene_group'])
...
def post_job():
    ...
    if hgnc_id is None or count is None:
        return "hgnc_id and count parameters required", 400
    if not isinstance(count, int):
        return "count must be integer > 0", 400
    if count <= 0:
        return "count must be > 0", 400

    job_dict = jobs.add_job(hgnc_id,count)
...
def get_results(jid):
    try:
        results_dict = jobs.get_results_by_id(jid)
        if results_dict:
